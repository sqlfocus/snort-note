...
static RNAServiceValidationPort pp[] =
{
    ...
    {&smtp_svc_validate, SMTPS_DEPRECATED_PORT, IPPROTO_TCP}, // here only to support old servers
    {NULL, 0, 0}
};
    ...
    if (getAppIdFlag(flowp, APPID_SESSION_ENCRYPTED | APPID_SESSION_DECRYPTED) == APPID_SESSION_ENCRYPTED)
    {
        if ((fd->flags & CLIENT_FLAG_STARTTLS_SUCCESS))
        {
...
#endif
            {
                ...
                clearAppIdFlag(flowp, APPID_SESSION_CLIENT_GETS_SERVER_PACKETS); // we no longer need to examine the response.
...
static int smtp_svc_validate(ServiceValidationArgs* args)
{
    ...
    if (dd->watch_for_deprecated_port)
    {
        ...
        if (args->pkt->src_port == SMTPS_DEPRECATED_PORT && !getAppIdFlag(flowp, APPID_SESSION_DECRYPTED))
        {
